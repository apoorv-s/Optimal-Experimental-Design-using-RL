...
def run_random_policy_benchmark(
    ...
    env = SensorOptimalPlacement(width=width, length=length, n_sensor=n_sensor,
                                  max_horizon=max_horizon, N_max=N_max, seed=seed)

    all_episode_rewards = []
    best_rewards = []
    optimal_states_all = []  # to store the optimal_states from each episode
    ...
    for episode_idx in range(num_episodes):
        ...
        optimal_states_all.append(info["optimal_state"])  # collect optimal_states
...
if __name__ == "__main__":
    ...
    all_episode_rewards, best_rewards, optimal_states_all = run_random_policy_benchmark()
